...
<div class="w-full max-w-3xl flex flex-col justify-center gap-2">
	...
	<div class="flex justify-between items-center w-full gap-4">
		<div>
			<button class="btn gap-2" onclick={() => showBranches = !showBranches} onblur="{(ev) => branchFocusOut(ev)}">
				<FontAwesomeIcon icon={faCodeBranch} size="lg" class="w-5 h-5" />
				<div class="s-hide">{data.branch}</div>
				<FontAwesomeIcon icon={faChevronDown} />
			</button>
			{#if showBranches}
				<div id="branch-list" class="z-10 bg-white divide-y divide-gray-100 rounded-lg shadow-lg w-44 absolute">
					<ul class="py-2 text-sm text-gray-700">
						{#each data.branches as branch}
							<li>
								<a href="../{branch}" class="block px-4 py-2 hover:bg-gray-100">{ branch }</a>
							</li>
						{/each}
					</ul>
				</div>
		...
		<div class="flex-grow text-sm text-gray-500 hover:text-gray-600">
			<a href="/{data.repo.owner.username}/{data.repo.name}/{data.branch}">{ data.message }</a>
		</div>
	...
	<div class="rounded-lg shadow bg-slate-50 w-full py-1">
		<table class="w-full text-gray-600 text-sm table-auto">
			...
			{#each data.files as file}
				<tr class="border-b h-10 hover:bg-gray-100">
					<td class="pl-3">
						<a href="{file.name}" class="flex flex-center items-center gap-1">
							{#if file.type === 'tree'}
								<FontAwesomeIcon icon={faFolder} size="lg" class="w-5" />
							{:else}
								<FontAwesomeIcon icon={faFile} size="lg" class="w-5" />
							{/if}
							<div class="w-200">{ file.name }</div>
						</a>
					</td>
